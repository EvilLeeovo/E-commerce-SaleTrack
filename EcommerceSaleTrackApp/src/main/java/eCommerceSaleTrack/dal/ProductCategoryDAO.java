...
public class ProductCategoryDAO {
    ...
    public Double getTotalFreightValueByCategory(String categoryName) throws SQLException {
        String query = "SELECT SUM(OrderItems.FreightValue) AS TotalFreightValue " +
        ...
        try (Connection connection = connectionManager.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    return rs.getDouble("TotalFreightValue"); // Return the total FreightValue for the specified category.
                }
            }
        }
        return null; // Return null if there is no matching data.
    }
